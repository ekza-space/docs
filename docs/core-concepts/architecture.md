...
- A **game release** pins one approved rendition for an avatar, game and profile.
  Publishing an update leaves it in place until that game accepts the update.
...
A game keeps its selected, published rendition until it approves another one.
Publishing a new revision does not interrupt existing game use. Each game may choose
a different revision. Withdrawing the selected submission removes that game's access
without falling back to an older approval. Global unpublish and account deletion
remove every game release. Curator review and game acceptance remain independent and
mandatory. Mirror's legacy feed still follows the current published iOS revision.
...
| --- | --- | --- |
| `GET /v2/avatars?project=&platform=&profile=` | A complete catalogue from both sources (503 on Studio outage) in a neutral shape: identifier, origin, renditions with absolute download URLs, hashes and sizes, approvals, and `access` (`free` or `owned`). With `project`, only what that game approved. Never cached, because publication is revocable. | Working |
...
- **A builder is isolated and its result verified.** Studio processors run in
  disposable containers without network access, with read-only inputs and code,
  resource limits and a writable directory for just that attempt. A configured,
  digest-pinned runtime image is required; no host execution fallback exists. The
  registry then checks the path, hash, size and profile requirements itself.
...
6. The isolated Omoba builder has been tested; the Blender/USDZ runtime image still
   needs preparation and verification before the worker changes can be deployed.
7. A game token lives in memory, so a player reconnects the account after restarting the
   game.
